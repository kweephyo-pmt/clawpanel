...
// ClawPanel -- Auto-detect environment and write .env.local
// Usage: npm run setup
...
async function main() {
  console.log()
  console.log(bold('  ClawPanel Setup'))
  ...
  if (httpEnabled === true) {
    ...
    console.log(`    ${dim('ClawPanel needs this to chat with agents.')}`)
  ...
  // Support --cwd flag for CLI usage (clawpanel setup writes .env.local into the package dir)
  const cwdFlag = process.argv.find((a) => a.startsWith('--cwd='))
  let targetDir = cwdFlag ? cwdFlag.split('=')[1] : process.cwd()

  // When installed globally, targetDir may not be writable.
  ...
  if (!canWriteToDir(targetDir)) {
    const userConfigDir = join(homedir(), '.config', 'clawpanel-ui')
  ...
  // Write
  const lines = [
    '# ClawPanel -- generated by npm run setup',
  ...
  console.log()
  const startCmd = cwdFlag ? 'npm run dev' : 'npm run dev'
  console.log(`  Next steps:`)
  if (!gatewayUp) {
    console.log(`    1. Start the gateway:  ${dim('openclaw gateway run')}`)
    console.log(`    2. Start ClawPanel:    ${dim(startCmd)}`)
  } else {
